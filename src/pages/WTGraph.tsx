...
import testGenes from '../data/WT_DEG.json'
    ...
    if(changeData === undefined){
      ...
      var mappedDatas: Plotly.PlotData[] = data.map((item: Partial<Plotly.PlotData>) => {
        return {
          ...
          name: item.name,
          mode: "markers",
          marker : {color: 'black'} //change color of scatter points in box and scatter
        };
      })as Plotly.PlotData[];
  ...
  */
  return (
    <><div className="App">
       ...
       <h4>Use the tool to explore RNA-seq data</h4>
        </header>
      </div>
      <div>
        ...
        <div className="searchbox-plotDropDown">
          <div className="searchbox">
            <div className="input-group">
             <input type="text" className="form-control" placeholder="Search gene of interest" onChange={(event) => setTempGene(event.target.value)} />
              <li className="listButtons" onClick={() => { updateGene(tempGene); }}>Update</li>
            </div>
          </div>
      ...
      <div className='tablescroll'><Reacttable jsonData={gene}></Reacttable></div>
    </div></>
  );
}

export default WTGraph;
